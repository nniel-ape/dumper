...
func (v *VaultStore) GetItem(id string) (*Item, error) {
	item := &Item{}
	var url, content, summary sql.NullString
	err := v.db.QueryRow(`
		SELECT id, type, url, title, content, summary, created_at, updated_at
		FROM items WHERE id = ?`, id,
	).Scan(&item.ID, &item.Type, &url, &item.Title, &content, &summary,
		&item.CreatedAt, &item.UpdatedAt)
...
func (v *VaultStore) ListItems(limit, offset int) ([]Item, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.Type, &url, &item.Title, &content,
			&summary, &item.CreatedAt, &item.UpdatedAt); err != nil {
...
func (v *VaultStore) ListItemsByTag(tag string, limit, offset int) ([]Item, error) {
	rows, err := v.db.Query(`
		SELECT DISTINCT i.id, i.type, i.url, i.title, i.content, i.summary, i.created_at, i.updated_at
		FROM items i
...
func (v *VaultStore) Search(query string, limit int) ([]SearchResult, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.Item.ID, &r.Item.Type, &url, &r.Item.Title,
			&content, &summary, &r.Item.CreatedAt, &r.Item.UpdatedAt,
