...
export function ItemDetail({ item, onBack, onTagClick }: ItemDetailProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-tg-bg flex flex-col">
      {/* Header */}
      <header className="shrink-0 px-4 py-3 border-b border-tg-hint/20 flex items-center gap-3">
        <button
          onClick={handleBack}
          className="p-1 -ml-1 text-tg-text hover:text-tg-link"
        >
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-4 pb-24">
        ...
        {item.summary && (
          <section>
            <h2 className="text-xs font-medium text-tg-hint uppercase tracking-wider mb-2">
              Summary
            </h2>
      ...
      {/* Actions */}
      <div className="absolute bottom-0 left-0 right-0 p-4 bg-tg-bg border-t border-tg-hint/20 flex gap-2 safe-area-bottom">
        {item.url && (
          <Button
            variant="default"
            className="flex-1"
